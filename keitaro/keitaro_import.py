...
# Целевой трекер (куда загружать)
KEITARO_TARGET_URL=https://your-new-tracker.com
KEITARO_TARGET_API_KEY=your-new-api-key

2. В скрипте keitaro_import.py укажите:
CONFIG = {
    # ...
    "IMPORT_TYPE": "offers",  # или "landings"
    "IMPORT_DIR": "путь_к_вашей_папке",  # например: "offer_exports_20250104"
    # ...
}
...
Что произойдет:
✅ Скрипт прочитает index.csv из вашей папки

✅ Для каждого оффера/лендинга:

Проверит, есть ли группа с таким названием
Если нет - автоматически создаст группу
Загрузит ZIP-архив через /import эндпоинт
Привяжет к правильной группе
✅ В конце покажет статистику:

Сколько импортировано
Сколько пропущено (если уже есть)
Ошибки (если были)
Настройки:
CREATE_GROUPS: True - создавать группы автоматически ✅
SKIP_EXISTING: True - пропускать если уже есть с таким именем
MATCH_BY_NAME: True - сопоставлять по именам
Скрипт уже всё учитывает! Просто укажите путь к папке с экспортом и запускайте 🚀
